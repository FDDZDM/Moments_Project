...
exports.getSystemStats = async (event, context) => {
  ...
  const totalUsers = await db.collection('users')
    .where({ status: dbCmd.neq(-1) })
    .count()
  
  // 今日活跃用户数（登录过的用户，且未删除）
  const todayActiveUsers = await db.collection('users')
    .where({ 
      last_login_time: dbCmd.gte(today.setHours(0, 0, 0, 0)),
      status: dbCmd.neq(-1)
    })
    .count()
  
  // 内容总数（排除已删除内容）
  const totalContents = await db.collection('contents')
  ...
  const todayContents = await db.collection('contents')
    .where({ 
      created_at: dbCmd.gte(today.setHours(0, 0, 0, 0)),
  ...
  const contentTrend = await Promise.all(dates.map(date => {
    ...
    return db.collection('contents')
      .where({
        ...
        status: dbCmd.neq('deleted') // 排除软删除
      })
      .count()
      .then(res => res.total)
  }))
